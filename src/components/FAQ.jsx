          ...
          <div className="flex flex-col w-full bg-[#FAFAFA]">
            {faqData.map((item, i) => (
              <div key={i} className={`${i < faqData.length - 1 ? 'border-b border-base-border' : ''} w-full`}>
                <button
                  className="w-full flex items-center justify-between p-6 text-left hover:bg-[#E0E0E0] transition-colors focus:outline-none"
                  onClick={() => toggle(i)}
                ...
                >
                  ...
                  <div
                    className="w-8 h-8 bg-accent-orange text-white flex items-center justify-center rounded-sm shrink-0 transition-transform"
                ...
                <div id={`faq-answer-${i}`} role="region" className={`accordion-content px-6 pb-0 text-sm text-gray-600 font-sans max-w-3xl ${activeIndex === i ? 'active' : ''}`}>
